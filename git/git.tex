...
\begin{frame}{Git}{Local and remote repositories}
	...
	\begin{itemize}
	\item \texttt{Working directory}: The files we work on
	\item \texttt{Repository}: Project files + a database with history of all changes (or \alert{commits})
		\begin{itemize}
...
\begin{frame}{Git}{Git overview (III)}
	\begin{columns}
		\column{.50\linewidth}
...
\begin{frame}{Using Git}{Initializing a repository}
	Using plain Git: \texttt{Git init}
		...
		\item By default, there are no remote
		\end{itemize}
	Using GitHub
		\begin{enumerate}
		\item We create a new repository in GitHub (which will be our remote)
...
\begin{itemize}
 ...
 % 		\texttt{git checkout <branch name>} switch branches\\
  ...
  \item There is a branch with name: HEAD
	\begin{itemize}
...
\begin{frame}[fragile]{Using Git}{Conflicts}
     Merging is quite a common operation in Git
	\begin{itemize}
		\item Changes in different parts of a file are automatically merged
	\end{itemize}

     Merging changes in the same part of a file cause \alert{conflicts}
	\begin{itemize}
		\item Git is no longer able to automatically merge
		\item Human intervention is required
	\end{itemize}
